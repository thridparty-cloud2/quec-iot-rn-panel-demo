...
import {QLog, TimeUtils} from '@quec/panel-sdk/utils'
...
const AlarmFaultPage: FC<AlarmFaultPageProps> = (props: AlarmFaultPageProps, context?: any) => {
   ...
   * 获取设备列表
   */
  const getAlarmList = (pageNum: number, refresh: boolean, isLoadMore: boolean) => {
    checkNetwork(() => {
      if (!refresh) {
        global.loading()
      }
      ...
      QuecRNUserModule.getUserMessageListWithParams(param)
        ...
        .catch((error: any) => {
          QLog.info('AlarmFaultPage', 'getAlarmList==' + JSON.stringify(error))
          if (!refresh) {
  ...
  const checkNetwork = (callback: Function) => {
    QuecRNNetworkModule.fetchPhoneInternetReachable()
      .then((res: any) => {
        if (res.internetReachable === 'true' || res.internetReachable === true) {
  ...
  const handlerData = (res: any, refresh: boolean) => {
    ...
    if (res.data && res.data.list.length > 0) {
      const newDataList = markDataList(res.data.list)
    ...
    return (
      <View style={alarmFaultPageStyle.recordContainerStyle}>
        <View style={alarmFaultPageStyle.timelineContainerStyle}>
          ...
          />
          <View
            style={[alarmFaultPageStyle.timelineDotStyle, {backgroundColor: colors.timelineColor}]}
          />
          <View
            style={
        ...
        <View style={alarmFaultPageStyle.recordContentContainerStyle}>
          ...
          <Text style={[alarmFaultPageStyle.timeTextStyle, {color: colors.hintTextColor}]}>
  ...
  if (showEmptyView) {
    ...
    return (
      <>
        <View style={[alarmFaultPageStyle.containerStyle, {backgroundColor: colors.bgColor}]}>
          <FlatList
