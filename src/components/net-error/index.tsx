...
export const NetError: FC<NetErrorProps> = ({
  ...
  return (
    <TouchableOpacity
      style={[styles.containerStyle, {backgroundColor: colors.bgColor}, containerStyle]}
      onPress={() => {
        onClick()
      }}
    >
      <Image source={icon ?? images.netError} style={styles.iconStyle} />
      <Text style={[styles.mainTipStyle, {color: colors.textColor}, mainTipStyle]}>
        {mainTip ?? i18n('net_error_page_main_tip')}
      </Text>
      <Text style={[styles.subTipStyle, {color: colors.hintTextColor}, subTipStyle]}>
