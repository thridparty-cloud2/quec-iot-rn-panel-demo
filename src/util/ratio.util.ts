...
export const convertX = (w: number) => (isWeb ? w : w * HRatio)
export const convertY = (h: number) => (isWeb ? h : h * VRatio)
export const convert = (number: number) => (isWeb ? number : number * ratio)
...
const getDimension = () => {
    if (isWeb) {
        try {
            return Dimensions.get('osWindow' as any)
        } catch (error) {
...
export default {
    ...
    get viewHeight() {
        return (
            getDimension().height -
            (isIos ? (isIphoneX ? 88 : 64) : 56 + statusHeight)
        )
    },
