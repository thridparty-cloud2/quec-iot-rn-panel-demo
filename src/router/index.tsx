...
import {useThemeStatusBarStyle} from '../hooks/theme'
...
export const PageRoutes = {
    DemoPage: new PageRouterImp(PAGES.PAGE_MAIN_DEMO, Main, {
        headerShown: false,
    }),
...
export default function AppContainer() {
    ...
    const barStyle = useThemeStatusBarStyle()
    ...
    return (
        <NavigationContainer>
            <Stack.Navigator
                initialRouteName={PAGES.PAGE_MAIN_DEMO}
                // @ts-ignore
                ...
                {routes.map(child => {
                    return (
                        <Stack.Screen
                            key={child.pageName}
                            name={child.pageName}
                            options={child.pageOptions}
                        >
                            {(props: any) => (
                                <child.pageClass
                                    {...props}
                                    {...(child.pageOptions?.props || {})}
                                />
                            )}
                        </Stack.Screen>
                    )
